...
class evolutionary_algorithm {
  public:
    class individual {
      public:
        ...
        individual trampoline_crossover(nutrition_facts &p, individual &rhs);
        double _second_order_mutation_strength = 0.4;
        double _crossover_probability = 0.6;
        double _mutation_strength = 0.4;

      private:
        void reflect(double &v, double lb, double ub);
    };

    using solution_ptr = std::shared_ptr<user_diet>;
    using individual_ptr = std::shared_ptr<user_diet>;
    ...
    enum class scaling_strategy {
        ...
        linear_rank, // fitness <- ((2-pressure_constant)/n_ind) + ((2 *
                     // rank(fx) * (pressure_constant - 1))/n_ind * (n_ind - 1))
  ...
  public:
    evolutionary_algorithm(nutrition_facts &p);
    ...
    void copy_solution(int refeicao);
  ...
  private /* methods */:
    ...
    population_type grasp(population_type &children);
  ...
  private /* members */:
    ...
    enum island_replacement_policy _island_replacement_policy =
    ...
    double _elitism_proportion = 0.02;
    // scaling
    ...
    enum selection_strategy _reproduction_selection_strategy =
        selection_strategy::overselection;
    ...
    // Dynamic nutrition
    population_type _hours_solutions;
    size_t _number_of_hours_solutions = 6;
};
